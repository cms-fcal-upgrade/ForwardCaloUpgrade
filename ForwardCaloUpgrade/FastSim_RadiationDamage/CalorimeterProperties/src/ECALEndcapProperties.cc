...
ECALEndcapProperties::ECALEndcapProperties(const edm::ParameterSet& fastDet)
{
  ...
  edm::ParameterSet myLight = fastDet.getParameter<edm::ParameterSet>("ECALRadiationDamage");
  radDamage.setLumi(myLight);
...
// radiation damage principle modification
double ECALEndcapProperties::lightCollectionEfficiency(double eta) const
{
  double lC = lightColl_;
  double relLY = radDamage.relativeLYLoss(eta);
  lC *= relLY;

  return lC;
}

double ECALEndcapProperties::lightCollectionUniformity(double eta) const
{
  double res = radDamage.constantTermLYLoss(eta);
  return sqrt( lightCollUnif_*lightCollUnif_ + res*res);
}

double ECALEndcapProperties::lightCollectionNonLinearity(double eta, double e) const
{
  return radDamage.nonlinearityLYLoss(eta,e);
}
