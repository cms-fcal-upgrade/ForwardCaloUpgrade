...
#include "G4UserPrimaryVertexAddendum.hh"
...
//----------------------
  ...
  tree_hcell = new TTree("HCell", "HCAL Cell energy info");
  tree_hcell->Branch("n_hcells",&n_hcells,"n_hcells/I");
  tree_hcell->Branch("e_hdep",  e_hdep,  "e_hdep[n_hcells]/D");
		
  tree_genvtx = new TTree("GenVertex", "Generator Level Vertex info");
  tree_genvtx->Branch("n_genvtx",&n_genvtx,"n_genvtx/I");
  tree_genvtx->Branch("genvtx_x", genvtx_x, "genvtx_x[n_genvtx]/D");
  tree_genvtx->Branch("genvtx_y", genvtx_y, "genvtx_y[n_genvtx]/D");
  tree_genvtx->Branch("genvtx_z", genvtx_z, "genvtx_z[n_genvtx]/D");
  tree_genvtx->Branch("genvtx_vtx", genvtx_vtx, "genvtx_vtx[n_genvtx]/I");
	
  tree_genpart = new TTree("GenParticle", "Generator Level Particle info");
  tree_genpart->Branch("n_genpart",&n_genpart,"n_genpart/I");
  tree_genpart->Branch("genpart_px", genpart_px,"genpart_x[n_genpart]/D");
  tree_genpart->Branch("genpart_py", genpart_py,"genpart_y[n_genpart]/D");
  tree_genpart->Branch("genpart_pz", genpart_pz,"genpart_z[n_genpart]/D");
...
// Fill Hcal cells
//----------------
   void HistoManager::FillHCells(G4int nhcells, G4double* p_cell)
   {
     n_hcells = nhcells;
     for( G4int ic=0; ic<n_hcells; ic++) { 
...
// Grab Generator level info from HepMC input file
//--------------------------------------------------------------
void HistoManager::GrabGenInfo(const G4Event* evt, G4int option)
{

	G4int vtxct = 0;
	G4int partct = 0;
	
	G4PrimaryVertex* Vrtx= evt->GetPrimaryVertex();

		
	while(Vrtx && (vtxct<100000)){
	  
	  G4UserPrimaryVertexAddendum* bcode = new G4UserPrimaryVertexAddendum();
	  bcode = (G4UserPrimaryVertexAddendum*)(Vrtx->GetUserInformation());
	  G4int vtx_id= (bcode->GetBarcode());

	  if (option == 1){
	    if( vtx_id ){
	      if((vtx_id) >-1000000){

		  genvtx_x[vtxct]= (Vrtx->GetX0());
		  genvtx_y[vtxct]=(Vrtx->GetY0()); 
		  genvtx_z[vtxct]=(Vrtx->GetZ0());
		  genvtx_vtx[vtxct]=(vtx_id);
			
		  vtxct++;
			
		  G4PrimaryParticle* prtcle = Vrtx->GetPrimary();

		  while( (prtcle != 0) && ( partct < 1000000) ){
				
		    genpart_vtx[partct]=(vtx_id);
		    genpart_px[partct]=prtcle->GetPx();
		    genpart_py[partct]=prtcle->GetPy();
		    genpart_pz[partct]=prtcle->GetPz();
		    genpart_id[partct]=prtcle->GetPDGcode();
 
		    partct++;
		    prtcle = prtcle->GetNext();
	   ...
	   if( vtx_id ){
	    ...
	    G4cout << "Selected invalid Vertex Record Option" << G4endl;
	  }
...
// Set Vertex Recording Option
//-------------------------------------------
  void HistoManager::SetRecVtxOption(G4int Value)
  { RecVtxOption = Value;}
