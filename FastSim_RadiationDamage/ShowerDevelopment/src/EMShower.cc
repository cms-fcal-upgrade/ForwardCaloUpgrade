...
{ 
  ...
  for ( unsigned int i=0; i<nPart; ++i ) {
    ...
    fotos.push_back( theECAL->photoStatistics() * 
		     theECAL->lightCollectionEfficiency( ((*thePart)[i])->eta() ) * 
		     theECAL->lightCollectionNonLinearity( ((*thePart)[i])->eta(), ((*thePart)[i])->e() )
		     );
    
    myUniformity.push_back(1. + theECAL->lightCollectionUniformity(((*thePart)[i])->eta())*
			   random->gaussShoot(0.,1.) );
...
EMShower::compute() {
  ...
  for (unsigned iStep=0; iStep<nSteps; ++iStep ) {
   ...
   // middle of the step
	...
	// Disable photo-statistics, it will be applied in RecHitMaker
	//	dE = (photos[i]>0.)? random->poissonShoot(dE*photos[i])/photos[i]:0.0;

	// all steps should be correlated, this is a constant term in resolution!
	// see calculation of myUniformity[i] per particle
	dE *= myUniformity[i];
