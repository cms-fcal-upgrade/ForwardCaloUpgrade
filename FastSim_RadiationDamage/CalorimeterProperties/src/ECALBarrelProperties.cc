...
double ECALBarrelProperties::lightCollectionEfficiency(double eta) const
{
...
double ECALBarrelProperties::lightCollectionUniformity(double eta) const
{
  double res = radDamage.constantTermLYLoss(eta);
  return sqrt( lightCollUnif_*lightCollUnif_ + res*res);
}

double ECALBarrelProperties::lightCollectionNonLinearity(double eta, double e) const
{
