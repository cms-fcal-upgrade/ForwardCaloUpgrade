  ...
  : random_(myrandom)
{
  ...
  noise_vec.resize(EBDetId::kSizeForDenseIndexing, noise_);
  radDamageFactor.resize(EBDetId::kSizeForDenseIndexing);
  radDamageEta.resize(EBDetId::kSizeForDenseIndexing);
...
void EcalBarrelRecHitsMaker::loadEcalBarrelRecHits(edm::Event &iEvent,EBRecHitCollection & ecalHits,EBDigiCollection & ecalDigis)
{
  ...
  for(unsigned ihit=0;ihit<nhit;++ihit)
      ...
      if(doDigis_)
	{
      ...
      if ( SRThreshold_ && energyDeposited < threshold_  && !isHighInterest(TThashedindex) && applyZSCells_[icell])
	{
...
//	  std::cout << " Creating " ;
      ...
      //      std::cout << " Updating " << TThashedindex << " " << energy << " " << sinTheta_[myDetId.ietaAbs()] <<std::endl;
       ...
       TTTEnergy_[TThashedindex]+=energyDeposited*sinTheta_[myDetId.ietaAbs()];
...
//  es.get<CaloTopologyRecord>().get(theCaloTopology);     
  ...
  for(unsigned ic=0; ic<size; ++ic) 
      ...
      if(sinTheta_[ietaAbs]==0.)
	{
      ...
      // eta for each crystal
      radDamageEta[crystalHashedIndex] = myEcalBarrelGeometry->getGeometry(myDetId)->getPosition().eta();
      radDamageFactor[crystalHashedIndex] = 1./radDamage.relativeLYLoss(radDamageEta[crystalHashedIndex]);
