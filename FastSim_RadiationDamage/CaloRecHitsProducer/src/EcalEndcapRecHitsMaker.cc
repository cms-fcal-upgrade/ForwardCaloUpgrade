  ...
  : random_(myrandom)
{
  ...
  edm::ParameterSet radiationParameters = RecHitsParameters.getParameter<edm::ParameterSet>("ECALRadiationDamage");
  radDamage.setLumi(radiationParameters);
  ...
  double photoStatistics_ = RecHitsParameters.getParameter<double>("photoStatistics");
  double lightColl_ = RecHitsParameters.getParameter<double>("lightColl");
  photoElePerGeV_  = photoStatistics_*lightColl_;

  radDamageFactor.resize(EEDetId::kSizeForDenseIndexing);
  radDamageEta.resize(EEDetId::kSizeForDenseIndexing);
...
void EcalEndcapRecHitsMaker::loadEcalEndcapRecHits(edm::Event &iEvent,EERecHitCollection & ecalHits,EEDigiCollection & ecalDigis)
{
  ...
  for(unsigned ihit=0;ihit<nhit;++ihit)
    {      
      ...
      // this is the energy corrected for calibration, not deposited energy
      float energy = theCalorimeterHits_[icell];

      // the deposited energy is reduced by Radiation Damage
      float energyDeposited = energy;
      if(radDamageFactor[icell] > 1.0) energyDeposited = energy/radDamageFactor[icell];
      ...
      if(doDigis_)
	{
	   ...
	   geVtoGainAdc(energyDeposited,gain,adc);
	   myDataFrame.setSample(0,EcalMGPASample(adc,gain));
	   //ecalDigis.push_back(myDataFrame);
	}

      // If the energy+noise is below the threshold, a hit is
      // nevertheless created, otherwise, there is a risk that a
      // "noisy" hit is afterwards put in this cell which would not be
      // correct.  the threshold is in amplitude, so the
      // intercalibration constant should be injected

      if ( energyDeposited<threshold_*((*ICMC_)[icell]) && !isHighInterest(myDetId) && applyZSCells_[icell]) 
	{
...
void EcalEndcapRecHitsMaker::loadPCaloHits(const edm::Event & iEvent)
{
  ...
  for (cficalo=colcalo->begin(); cficalo!=cficaloend;cficalo++) 
      ...
      if(theCalorimeterHits_[hashedindex]==0.)
	{
	  ...
	  if (!noisified_ ) {
	    // noise is boosted by radiation damage
	    theCalorimeterHits_[hashedindex] += radDamageFactor[hashedindex] * calib * 
	                                        random_->gaussShoot(0., noise); 
	  }
	}

      float energy=(cficalo->energy()==0.) ? 0.000001 : cficalo->energy() ;

      // First, energy fluctuation due to photo-statistics (it was disabled in EMShower.cc)
      double pe = cficalo->energy() * photoElePerGeV_ / radDamageFactor[hashedindex];
      energy=(pe>0.)? cficalo->energy() * random_->poissonShoot(pe)/pe : 0.0;

      // cficalo->energy can be 0 (a 7x7 grid is always built) in this
      // case, one should not kill the cell (for later noise
      // injection), but it should be added only once.  This is a
      // dirty trick.
      ...
      if(TTTEnergy_[TThashedindex]==0.)
	{
      ...
      // TTTEnergy is the trigger related value. The trigger is set at
      // Rad.Damage=0 to be just above the noise. Assume that we use
      // the similar approach with Radiation Damage
      float energyDeposited = energy;
      if(radDamageFactor[hashedindex]>1.0) energyDeposited /= radDamageFactor[hashedindex];

      TTTEnergy_[TThashedindex]+=energyDeposited*sinTheta_[(hashedindex<EEDetId::kEEhalf)? hashedindex : hashedindex-EEDetId::kEEhalf];
	  ...
	  if(theCalorimeterHits_[hashedindex]==0.)
	    {
	      ...
	      // noise is boosted by radiation damage for calibrated hit
	      theCalorimeterHits_[hashedindex] = energy * radDamageFactor[hashedindex];
	      theFiredCells_.push_back(hashedindex);
	      ...
	      if(TTTEnergy_[newtthi]==0.)
		{
	      ...
	      // this is visible energy in TT, don't boost it
...
void EcalEndcapRecHitsMaker::randomNoisifier()
{
  ...
  while(icell < ncells)
      ...
      if(theCalorimeterHits_[cellindex]==0.)
	{
	  ...
	  energy *= calib;
	  theCalorimeterHits_[cellindex]=energy*radDamageFactor[cellindex];	  
...
void EcalEndcapRecHitsMaker::init(const edm::EventSetup &es,bool doDigis,bool domiscalib)  
{
  ...
  radDamageEta.resize(size);
  radDamageFactor.resize(size);
  ...
  for(unsigned ic=0; ic<size; ++ic) 
	...
	TTofSC_[schi].push_back(tthashedindex);
      ...
      // Create radiation damage
      //
      radDamageEta[cellhashedindex] = myEcalEndcapGeometry->getGeometry(myDetId)->getPosition().eta();
      radDamageFactor[cellhashedindex] = 1./radDamage.relativeLYLoss(radDamageEta[cellhashedindex]);
...
bool EcalEndcapRecHitsMaker::isHighInterest(const EEDetId& detid)
{
  ...
  for(unsigned itt=0;itt<size&&!result;++itt)
    {
      //      std::cout << " Checking TT " << tts[itt] << " Energy=" << TTTEnergy_[tts[itt]] 
      //		<< " SRThreshold=" << SRThreshold_ << std::endl;
