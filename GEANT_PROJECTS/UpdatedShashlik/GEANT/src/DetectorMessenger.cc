...
{ 
  ...
  XOffsetCmd = new G4UIcmdWithADoubleAndUnit("/ecal/det/setXCalOffset",this);  
  XOffsetCmd->SetGuidance("set offset of center of calorimeter along the X direction.");
  XOffsetCmd->SetParameterName("offsetX",false);
  XOffsetCmd->SetUnitCategory("Length");
  XOffsetCmd->AvailableForStates(G4State_PreInit,G4State_Idle);  

  YOffsetCmd = new G4UIcmdWithADoubleAndUnit("/ecal/det/setYCalOffset",this);  
  YOffsetCmd->SetGuidance("set offset of center of calorimeter along the Y direction.");
  YOffsetCmd->SetParameterName("offsetY",false);
  YOffsetCmd->SetUnitCategory("Length");
  YOffsetCmd->AvailableForStates(G4State_PreInit,G4State_Idle);  
...
void DetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  if( command == XOffsetCmd )
   { Detector->SetECalXOffset(XOffsetCmd->GetNewDoubleValue(newValue));}
   
  if( command == YOffsetCmd )
   { Detector->SetECalYOffset(YOffsetCmd->GetNewDoubleValue(newValue));}
  ...
  if( command == NbCellHcalCmd )
   { Detector->SetHcalCells(NbCellHcalCmd->GetNew3VectorValue(newValue));}
  ...
  if( command == BeamholeCmd )
    { Detector->SetBeamholeOption(BeamholeCmd->GetNewIntValue(newValue));}
